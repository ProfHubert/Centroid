...
//Initialization
	...
	for(int iteration=0;iteration<=iterationNum;iteration++)
	{
		...
		ra = (sqrt(nCols * nRows / seedNum / 2) - 1) / 2;
        radiu = floor(ra);
		...
		for(int i=0;i<seedNum;i++)
		{
			...
			minX=(x-radiu<=0)?0:x-radiu;
			minY=(y-radiu<=0)?0:y-radiu;
			maxX=(x+radiu>=nRows-1)?nRows-1:x+radiu;
			maxY=(y+radiu>=nCols-1)?nCols-1:y+radiu;
			...
			centerL1[i] = centerL1[i]/area;
			centera1[i] = centera1[i]/area;
			centerb1[i] = centerb1[i]/area;
			
			centerL2[i] = centerL2[i]/area;
			centera2[i] = centera2[i]/area;
			centerb2[i] = centerb2[i]/area;
